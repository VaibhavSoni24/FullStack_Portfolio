# Vaibhav Soni - Full Stack Developer Portfolio

A production-grade, minimal, Modern-style portfolio specifically designed for Full Stack Software Engineering pathway.

## Overview

This portfolio showcases enterprise-grade projects, technical expertise, and professional experience with a focus on:
- Backend & Cloud Engineering
- Systems Architecture
- Full Stack Development
- Clean, minimal design

## Tech Stack

- **Framework**: Next.js 14 (App Router)
- **Language**: JavaScript
- **Styling**: CSS Modules
- **Icons**: Lucide React
- **Fonts**: Inter
- **Image Optimization**: Next.js Image

## Getting Started

### Prerequisites

- Node.js 18.x or higher
- npm or yarn

### Installation

1. Clone the repository:
```bash
git clone <repository-url>
cd FullStackPortfolio
```

2. Install dependencies:
```bash
npm install
```

3. Run the development server:
```bash
npm run dev
```

4. Open [http://localhost:3000](http://localhost:3000) in your browser.

## Project Structure

```
FullStackPortfolio/
├── public/
│   └── assets/          # Images and resume
├── src/
│   ├── app/
│   │   ├── layout.js    # Root layout with metadata
│   │   ├── page.js      # Main page
│   │   └── globals.css  # Global styles
│   └── components/      # React components
│       ├── Hero.js
│       ├── Experience.js
│       ├── Projects.js
│       ├── Achievements.js
│       ├── Education.js
│       ├── Skills.js
│       ├── Certificates.js
│       ├── Languages.js
│       └── Footer.js
├── package.json
└── next.config.js
```

## Assets Required

Place the following files in the `public/assets/` directory:

- `photo.jpeg` - Professional photo
- `Vaibhav_Soni_FullStack_Resume.pdf` - Resume
- `CollegeTips.in-WebDevelopment-2025.jpg` - Internship certificate
- `sphinx.jpeg` - Sphinx hackathon image
- `digifest.jpeg` - TiE Global Summit image

## Design Principles

- **Minimal & Clean**: No unnecessary decorations
- **Professional**: Modern SaaS dashboard aesthetic
- **Accessible**: Semantic HTML and proper contrast
- **Responsive**: Mobile-first approach
- **Performance**: Optimized images and code splitting

## Color System

- Background: White (#ffffff)
- Secondary Background: Light Gray (#f8f9fa)
- Text: Near-black (#1a1a1a)
- Secondary Text: Gray (#4a5568)
- Accent: Dark Charcoal (#2d3748)
- Border: Light Gray (#e2e8f0)

## Features

- ✅ SEO optimized with meta tags
- ✅ Fully responsive design
- ✅ Performance optimized
- ✅ Accessible UI
- ✅ Clean code structure
- ✅ Production-ready

## License

© 2026 Vaibhav Soni. All rights reserved.

## Contact

- Email: vaibhavsoni280506@gmail.com
- Phone: +91 6350366202
- GitHub: [VaibhavSoni24](https://github.com/VaibhavSoni24)
- LinkedIn: [Vaibhav Soni](https://www.linkedin.com/in/vaibhav-soni-867836285/)
